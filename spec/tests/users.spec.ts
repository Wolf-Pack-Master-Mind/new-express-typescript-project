...
const msgs = {
  getUsersSuccess:
    "should return a JSON object with all the users and a " +
  ...
  addUserFailedMissingParam:
    "should return a JSON object with an error " +
  ...
  addUserFallbackErr:
    ...
    "was unsuccessful.",
  updateSuccess:
    `should return a status code of "${HttpStatusCodes.OK}" if ` +
  ...
  updateFallbackErr:
    ...
    "was unsuccessful.",
  deleteSuccessful:
    `should return a status code of "${HttpStatusCodes.OK}" ` +
...
describe("user-router", () => {
  ...
  describe(`"DELETE:${deleteUserPath}"`, () => {
    ...
    it(msgs.deleteUserNotFound, (done) => {
      callApi("horse" as unknown as number).end((err: Error, res: Response) => {
        !!err && logger.err(err);
        expect(res.status).toBe(HttpStatusCodes.BAD_REQUEST);
        expect(res.body.error).toBe(defaultErrMsg);
        done();
